...
!      ENDDO
   ...
   SUBROUTINE colm_500m_to_30m(grid_in, block_500m, block_30m)
      ...
      type(block_data_real8_2d), intent(in)    :: block_500m
      type(block_data_real8_2d), intent(inout) :: block_30m

      integer :: iblkme, jblkme
      integer :: ib, jb, i, j, is, ie, js, je

      DO iblkme = 1, gblock%nblkme
         ib = gblock%xblkme(iblkme)
         jb = gblock%yblkme(iblkme)

         DO j = 1, grid_in%ycnt(jb)
            DO i = 1, grid_in%xcnt(ib)

               is = 1 + (i-1)*15
               ie = i*15
               js = 1 + (j-1)*15
               je = j*15
               block_30m%blk(ib,jb)%val(is:ie,js:je) = block_500m%blk(ib,jb)%val(i,j)

            ENDDO
         ENDDO
      ENDDO
   END SUBROUTINE colm_500m_to_30m

   SUBROUTINE colm_1km_to_500m(pixelset, grid_in1, grid_in2, block_in, block_out)
      ...
      type (pixelset_type), intent(in) :: pixelset
      type(grid_type), intent(in) :: grid_in1, grid_in2
      type(block_data_real8_2d), intent(in)    :: block_in
      type(block_data_real8_2d), intent(inout) :: block_out

      integer :: iblkme
      integer :: ie, ipxstt, ipxend, npxl, npatch
      integer :: iset, ipxl, xblk, yblk, xloc1, xloc2, yloc1, yloc2
      integer,  allocatable :: ylist1(:), xlist1(:), ylist2(:), xlist2(:)

      npatch = pixelset%nset
      ...
      DO iset = 1, npatch
         ...
         DO ipxl = ipxstt, ipxend
            ...
            xblk  = grid_in1%xblk(xlist1(ipxl))
            yblk  = grid_in1%yblk(ylist1(ipxl))

            xloc1 = grid_in1%xloc(xlist1(ipxl))
            yloc1 = grid_in1%yloc(ylist1(ipxl))

            xloc2 = grid_in2%xloc(xlist2(ipxl))
            yloc2 = grid_in2%yloc(ylist2(ipxl))

            print*, block_in%blk(xblk,yblk)%val(xloc1,yloc1)
            block_out%blk(xblk,yblk)%val(xloc2,yloc2) = block_in%blk(xblk,yblk)%val(xloc1,yloc1)
         ENDDO
...
!         jb = gblock%yblkme(iblkme)
!
!         print *, 'Shape of val = ', shape(block_1km%blk(ib,jb)%val)
!         print *, 'Shape of val = ', shape(block_500m%blk(ib,jb)%val)
!         print*, 'ib ', ib, 'jb ', jb
!         DO j = 1, grid_in%ycnt(jb)
!            DO i = 1, grid_in%xcnt(ib)
!
...
!               print*, 'is:ie: ', is, ie
!               print*, 'js:js: ', js, je
!               block_500m%blk(ib,jb)%val(is:ie,js:je) = block_1km%blk(ib,jb)%val(i,j)
!
!            ENDDO
!         ENDDO
!      ENDDO
   END SUBROUTINE colm_1km_to_500m
