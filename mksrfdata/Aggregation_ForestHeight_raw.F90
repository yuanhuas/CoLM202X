...
!-----------------------------------------------------------------------
   ...
   integer :: typpc   (N_land_classification+1)
      ...
      IF (p_is_io) THEN
         ...
         print*, 'Reading urban_htop from vname: ', trim(DEF_rawdata%urban_htop%vname)
         print*, 'Reading htop from vname: ', trim(DEF_rawdata%htop%vname)
         CALL read_5x5_data     (dir_5x5, fname, gland   , trim(DEF_rawdata%htop%vname), htop)! 路径，后缀，网格，变量名，输出数据
! trim()去除字符串的尾部空格，DEF_rawdata%htop%vname多层派生类型访问
